    ...
    <div className="flex flex-col h-full min-h-0 overflow-auto bg-white text-neutral-900">
      ...
      <div className="flex items-center gap-4 px-5 pt-3.5 pb-3 border-b-2 border-neutral-200 flex-none">
        <div className="flex flex-col gap-1">
          ...
          <span className="text-[11px] text-neutral-500">Clock starts at the queue geofence and stops at barrier release · exclusions recorded per visit, never hidden</span>
        </div>
        <div className="flex ml-3">
          {[["visits","Live visits"],["appts","Appointments"]].map(([k,label],i,arr)=>(
        ...
        <div className="ml-auto">
          <Button size="sm" className="text-xs" onClick={()=>setAdvanced(true)}>
            {advanced?"V-2043 served · gate pass issued":"Check in next in queue"}
          </Button>
        </div>
      </div>
            ...
            <div className="border-t-2 border-neutral-200">
              {[
                ...
                <div key={t.k} className="flex gap-3 items-baseline px-4 py-2 border-b border-neutral-200 text-[11.5px]">
                  <span className="w-2 h-2 flex-none inline-block rounded-sm" style={{background:!t.v?"#e5e7eb":i===5?"#d9291c":"#1f2937"}} />
            ...
            <div className="px-4 pb-2.5">
              ...
              <div className="flex gap-1 mt-2">
                {["front","left","right","rear"].map(p=>(
                  <div key={p} className="flex-1 h-11 bg-neutral-200 border border-neutral-400 flex items-end p-0.5 text-[9px] text-neutral-600">{p}</div>
                ))}
              </div>
            </div>
            <div className="px-4 pt-3 pb-1.5 text-[10px] tracking-widest uppercase text-neutral-500 font-bold">Actions</div>
            <div className="flex flex-col gap-1.5 px-4 pb-4">
              <Button size="sm" className="text-[11.5px] justify-start" onClick={()=>setAdvanced(true)}>Check in and assign lane</Button>
              <Button variant="secondary" size="sm" className="text-[11.5px] justify-start">Capture EIR photos</Button>
              <Button variant="secondary" size="sm" className="text-[11.5px] justify-start">Record exclusion reason</Button>
                  ...
                  <div className="flex gap-0.5 flex-1">
                    {Array.from({length:Math.max(a.capacity,a.booked)},(_,i)=>(
                      <span key={i} className="w-6 h-4 border inline-block"
                        style={{ background:i<a.booked?(i>=a.capacity?"#d9291c":"#1f2937"):"transparent", borderColor:i>=a.capacity?"#d9291c":"#6b7280" }} />
                    ))}
                  </div>
