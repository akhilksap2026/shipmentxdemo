...
function AppShell() {
  ...
  // ── Step 1: Persona dropdown state ───────────────────────────────────────
  const [personaOpen, setPersonaOpen] = useState(false)
  const personaRef = useRef<HTMLDivElement>(null)

  // ── Step 2: Collapsible sidebar groups ───────────────────────────────────
  ...
  // ── Step 3: Collapsible story bar ─────────────────────────────────────────
  const [storyExpanded, setStoryExpanded] = useState(false)

  useEffect(() => { localStorage.setItem("yardos:showDemo", String(showDemo)) }, [showDemo])

  // Auto-expand the group for the active screen whenever screen changes
  useEffect(() => {
    ...
    <>
      ...
      <div
        ...
        style={{
          ...
          gridTemplateRows: showDemo ? "44px 34px minmax(0,1fr)" : "44px 0px minmax(0,1fr)",
        }}
      >
        {/* ── Sidebar ──────────────────────────────────────────────────────── */}
        <div
        ...
        >
          ...
          <div className="px-3 py-2.5">
            <button
              ...
              style={{ background: "rgba(255,255,255,0.06)", border: "1px solid rgba(255,255,255,0.06)", fontSize: 11, color: "#6b7280" }}
            >
            ...
            return (
              <div key={group} className="mt-1">
                ...
                <button
                  ...
                  className="w-full flex items-center gap-1 px-4 py-1.5 text-left"
                  style={{ color: hasActive ? "#9ca3af" : "#4b5563" }}
                >
                  <span className="ds-label flex-1" style={{ color: "inherit" }}>{group}</span>
                  <span style={{ fontSize: 8, opacity: 0.6 }}>{isExpanded ? "▲" : "▼"}</span>
                </button>
        ...
        >
          ...
          <div className="ml-auto flex items-center gap-2.5">
            ...
            {/* ── Live sync — compact dot + "Live" with hover tooltip ── */}
            <div className="relative group">
              ...
              <div
                ...
                style={{ background: "#111827", color: "#e5e7eb", fontSize: 10.5, padding: "4px 10px", borderRadius: 4, whiteSpace: "nowrap", zIndex: 50 }}
              >
                Last sync: {syncLabel}
              </div>
            </div>
            ...
            {!backendConnected && (
              <button
                ...
                style={{ fontSize: 11, background: reconnecting ? "transparent" : "#fef2f2", border: "1px solid #fecaca", color: "#dc2626", borderRadius: 5 }}
              >
            ...
            <div ref={personaRef} className="relative">
              <button
                ...
                style={{ fontSize: 11, background: "#111827", color: "#fff", border: "1px solid #374151", borderRadius: 5 }}
              >
                <span
                  className="flex-none flex items-center justify-center font-black"
                  style={{ width: 18, height: 18, background: "#dc2626", borderRadius: 2, fontSize: 10 }}
                >{p.name[0]}</span>
                {p.name}
                <span style={{ fontSize: 8, opacity: 0.6, marginLeft: 1 }}>{personaOpen ? "▲" : "▼"}</span>
              </button>

              {personaOpen && (
                <div
                  className="absolute right-0 top-full mt-1 z-50"
                  style={{ background: "#1f2937", border: "1px solid #374151", borderRadius: 5, overflow: "hidden", minWidth: 192 }}
                >
                  {PERSONAS.map(px => (
                    <button
                      key={px.id}
                      ...
                      style={{
                        fontSize: 11,
                        color: persona === px.id ? "#fff" : "#9ca3af",
                        fontWeight: persona === px.id ? 600 : 400,
                        background: persona === px.id ? "rgba(220,38,38,0.12)" : "transparent",
                        borderLeft: `2px solid ${persona === px.id ? "#dc2626" : "transparent"}`,
                      }}
                      onMouseEnter={e => { if (persona !== px.id) (e.currentTarget as HTMLElement).style.background = "rgba(255,255,255,0.04)" }}
                      onMouseLeave={e => { (e.currentTarget as HTMLElement).style.background = persona === px.id ? "rgba(220,38,38,0.12)" : "transparent" }}
                    >
                      <span
                        className="flex-none flex items-center justify-center font-black"
                        style={{ width: 20, height: 20, background: persona === px.id ? "#dc2626" : "#374151", borderRadius: 2, fontSize: 10 }}
                      >{px.name[0]}</span>
                      <div className="flex flex-col gap-0.5">
                        <span>{px.name}</span>
                        <span style={{ fontSize: 9.5, color: "#6b7280", fontWeight: 400 }}>{px.sub}</span>
                      </div>
                    </button>
                  ))}
                </div>
            ...
            <button
              ...
              disabled={refreshing}
              className="px-3 py-1.5 font-medium disabled:opacity-50"
              style={{ fontSize: 11, background: "transparent", border: "1px solid #e5e7eb", color: "#374151", borderRadius: 5 }}
            >
            ...
            <button
              ...
              style={{ fontSize: 11, background: showDemo ? "#111827" : "transparent", border: "1px solid #e5e7eb", color: showDemo ? "#fff" : "#9ca3af", borderRadius: 5 }}
            >
              🎬 Demo
            </button>

            {/* Bell */}
            <button aria-label="Notifications" className="px-1.5" style={{ fontSize: 14, color: "#9ca3af" }} onClick={() => {}}>
              🔔
            </button>
          </div>
        </div>

        {/* ── Story bar — collapsible ───────────────────────────────────────── */}
        {showDemo && (
          <div
            className="col-start-2 flex items-center gap-2.5 px-3"
            style={{ background: "#fffbeb", borderBottom: "2px solid #fcd34d", height: 34, overflow: "hidden" }}
          >
            ...
            {/* Nav buttons — always visible (compact arrows when collapsed, labeled when expanded) */}
        ...
        <div className="col-start-2 row-start-3 min-w-0 min-h-0 overflow-hidden relative" style={{ background: "#f4f5f7" }}>
          {!ok ? (
            <div className="h-full flex items-start p-5">
              <div className="p-4" style={{ background: "#fff", border: "1px solid #e5e7eb", maxWidth: 420 }}>
